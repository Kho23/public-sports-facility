    ...
    <div className="bg-white min-h-screen font-sans">
      <div className="max-w-6xl mx-auto p-6">
          ...
          <div>
            <div className="flex items-center gap-2 mb-2">
              <span className={`inline-block px-3 py-1 rounded-full text-xs font-bold tracking-wide
                ...
                {lesson.category === '골프장' ? '골프' : lesson.category === '풋살' ? '풋살' : lesson.category === '헬스장' ? '헬스' : lesson.category === '무용실' ? '무용' : '해당없음'}
              </span>
        ...
        <div className="border border-gray-300 rounded-xl p-6 mb-8 bg-white shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6 mb-8">
            <div className="flex items-start gap-3">
              <div className="mt-1 bg-gray-100 p-2 rounded-lg text-gray-600"><User className="w-5 h-5" /></div>
              <div>
            ...
            <div className="flex items-start gap-3">
              <div className="mt-1 bg-gray-100 p-2 rounded-lg text-gray-600"><MapPin className="w-5 h-5" /></div>
              <div>
                <p className="text-sm font-bold text-gray-500 mb-1">장소/시설</p>
                <p className="text-gray-900 font-semibold text-lg">
                  {lesson.facilityType || '시설 정보'} <span className="text-sm font-normal text-gray-500 ml-2">({lesson.category})</span>
                </p>
              </div>
            </div>
            <div className="flex items-start gap-3">
            ...
            <div className="flex items-start gap-3">
              <div className="mt-1 bg-gray-100 p-2 rounded-lg text-gray-600"><Clock className="w-5 h-5" /></div>
              <div>
                <p className="text-sm font-bold text-gray-500 mb-1">교육 시간</p>
                <p className="text-gray-900 font-semibold text-lg">{lesson.startTime?.substring(0, 5)} ~ {lesson.endTime?.substring(0, 5)}</p>
              </div>
            </div>
          </div>
          ...
          <div className="mb-8 p-4 bg-red-50 border border-red-100 rounded-lg flex justify-between items-center">
            ...
            <span className="text-3xl font-extrabold text-red-600">
              {lesson.price ? Number(lesson.price).toLocaleString() : 0} 원
            </span>
          </div>
        ...
        <div className="flex justify-end gap-3 pb-20">
          ...
          {!isLoggedIn ? (
            <button
              className="px-8 py-3 rounded-lg font-bold text-white bg-gray-400 cursor-not-allowed h-[48px]"
              disabled
            >
              로그인 후 신청 가능
            </button>
