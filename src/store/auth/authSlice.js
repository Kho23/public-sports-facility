...
export const loginAsync = createAsyncThunk(
    ...
    async (loginInfo, { rejectWithValue }) => {
        try {//loginInfo 는 아이디와 패스워드가 담긴 객체
...
export const registerAsync = createAsyncThunk(
    ...
    async (memberInfo, { rejectWithValue }) => {
        try { //회원가입 시 입력된 정보가 memberInfo 에 객체 형태로 담김
...
//슬라이스 생성
const authSlice = createSlice({ //createSlice 로 생성되면 useSelector 를 통해 상태값을 불러올수 있다 
    name: "authSlice",
    ...
    extraReducers: (builder) => {
        ...
        //로그인 버튼이 눌리면 loginAsync 가 실행되어 그 상태(fulfilled, rejected, pending)에 따라 처리합니다
            ...
            .addCase(loginAsync.fulfilled, (state, action) => {//로그인 fulfilled 상태일때 처리(성공)
                console.log("로그인 성공")
